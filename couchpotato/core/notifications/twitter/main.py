...
class Twitter(Notification):
        ...
        try:
            if direct_message:
                ...
                if len(update_message) > 140:
                    if mention_tag:
                        api.PostUpdate(update_message[:135 - len(mention_tag)] + f'{mention_tag} 1/2 ')
                        api.PostUpdate(update_message[135 - len(mention_tag):] + f'{mention_tag} 2/2 ')
                    else:
    ...
    def getAuthorizationUrl(self, host = None, **kwargs):
        ...
        callback_url = (
            cleanHost(host)
            + f"{Env.get('api_base').lstrip('/')}notify.{self.getName().lower()}.credentials/"
        )
